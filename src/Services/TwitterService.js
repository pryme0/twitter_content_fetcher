...
class TwitterService {
    ...
    static async getUserContent(user) {
        ...
        return new Promise((resolve, reject) => {
            TWI.get('search/tweets', { q: 'javascript OR boobs since:2020-09-01', count: 30 }, function(err, data, response) {
                if (err) {
                    return reject(`Error getting conztent : ${JSON.stringify(error)}`, 500);
                } else {
                    return resolve(data)
                }
            })
